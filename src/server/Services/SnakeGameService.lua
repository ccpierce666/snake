...
local StateSnapshotSignal = Knit.CreateSignal()
...
local SnakeGameService = Knit.CreateService {
    ...
    Client = {
        ...
        StateSnapshot = StateSnapshotSignal,
    },
}
...
local leaderboardDirty = false
local nextLeaderboardPushAt = 0
local LEADERBOARD_PUSH_INTERVAL = 0.15
local playerMoney = {}
local playerSpins = {}
...
local markLeaderboardDirty
local pushLeaderboardState
local buildRealtimeSnapshot
...
function SnakeGameService:SubscribeRealtime(player)
    return buildRealtimeSnapshot(player)
end
...
markLeaderboardDirty = function()
    leaderboardDirty = true
end

pushLeaderboardState = function(forcePush)
    local now = os.clock()
    if not forcePush then
        if not leaderboardDirty then return end
        if now < nextLeaderboardPushAt then return end
    end
    leaderboardDirty = false
    nextLeaderboardPushAt = now + LEADERBOARD_PUSH_INTERVAL
